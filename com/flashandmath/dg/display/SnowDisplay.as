package com.flashandmath.dg.display {
	...
	public class SnowDisplay extends Sprite {
		...
		private var aveRed:Number;
		private var aveBlue:Number;
		...
		function SnowDisplay(w = 550, h = 300):void {
			...
			if (display.numOnStage < maxNumParticles) {
				...
				if (count >= waitCount) {
					...
					for (var i:int = 0; i <= particlesToAddEachFrame-1; i++) {
						...
						var thisParticle = display.addParticle(thisX,
														-particleSize/2,
														0.1*(Math.random()*2-1),
						...
						thisParticle.blendMode = BlendMode.LIGHTEN;
